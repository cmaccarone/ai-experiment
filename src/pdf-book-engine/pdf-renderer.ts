...
// Words containing these patterns trigger fontkit's GSUB ligature substitution,
// which creates visual gaps in the PDF. We render these words char-by-char.
const LIGATURE_PATTERN = /f[filft]/;
...
export class PdfRenderer {
  ...
  private renderLine(page: PDFPage, line: LayoutLine, pageHeight: number): void {
    ...
    // Render each token. Words containing ligature pairs (fi, fl, ff) are
    // drawn char-by-char to avoid fontkit's buggy GSUB ligature substitution
    // which creates visual gaps. Other words use single drawText for performance.
    for (const w of words) {
      if (/^\s+$/.test(w.text)) {
        x += spaceGap;
      } else {
        const font = this.writer.getFont(w.fontStyle);
        if (LIGATURE_PATTERN.test(w.text)) {
